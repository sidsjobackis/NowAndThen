...
import se.miun.nowandthen.gui.RadarPoints;
import se.miun.nowandthen.gui.ScreenLine;
import se.miun.nowandthen.render.Camera;
...
public class DataView {
	...
	public void requestData(String url,DATAFORMAT dataformat, String datasource) {
...
//			else {
				...
				for(String name : NameHelper.NAMES) {
					if (mixContext.isDataSourceSelected(name))
						searchQuery += "%20NOT%20itemLabel=\""+name+"\"";
				}
				
				requestData(DataSource.createRequestURL(lat,lon,alt,radius,searchQuery), DATAFORMAT.TEST, "TEST");  // TODO Kan bli fel här kanske?!?!
						// Debug notification
...
//			}
			...
			while((dRes=dm.getNextResult())!=null)
			{
				if (dRes.error && retry < 5) {
					...
					Log.d("Danger! Danger!", mixContext.getResources().getString(R.string.download_error));
				...
				if(!dRes.error) {
					...
					// Notification
					Toast.makeText(mixContext, mixContext.getResources().getString(R.string.download_received) + " " + mixContext.getResources().getString(R.string.ksam), Toast.LENGTH_SHORT).show();
